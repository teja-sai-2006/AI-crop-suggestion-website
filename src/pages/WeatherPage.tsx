        ...
        <main className="container mx-auto px-6 py-8 pt-20">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium mb-2">Select Your Location</h3>
      ...
      <main className="container mx-auto px-6 py-8 pt-20">
        <div className="space-y-8">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card onClick={() => navigate('/weather/temperature')} className="cursor-pointer hover:shadow-glow">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium flex items-center gap-2">Temperature
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{isLocationSet ? `${weatherData.current.temperature}°C` : '--°C'}</div>
                  <p className="text-xs text-muted-foreground">{isLocationSet ? `Feels like ${weatherData.current.feelsLike}°C` : 'Select location'}</p>
                </CardContent>
              </Card>

              <Card onClick={() => navigate('/weather/humidity')} className="cursor-pointer hover:shadow-glow">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium flex items-center gap-2">Humidity
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{isLocationSet ? `${weatherData.current.humidity}%` : '--%'}</div>
                  <p className="text-xs text-muted-foreground">{isLocationSet ? getHumidityLevel(weatherData.current.humidity) : 'Select location'}</p>
                </CardContent>
              </Card>

              <Card onClick={() => navigate('/weather/wind')} className="cursor-pointer hover:shadow-glow">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium flex items-center gap-2">Wind Speed
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{isLocationSet ? `${weatherData.current.windSpeed} km/h` : '-- km/h'}</div>
                  <p className="text-xs text-muted-foreground">{isLocationSet ? weatherData.current.windDirection : 'Select location'}</p>
                </CardContent>
              </Card>

              <Card onClick={() => navigate('/weather/uv-index')} className="cursor-pointer hover:shadow-glow">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium flex items-center gap-2">UV Index
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{isLocationSet ? weatherData.current.uvIndex : '--'}</div>
                  <p className="text-xs text-muted-foreground">{isLocationSet ? getUVRisk(weatherData.current.uvIndex) : 'Select location'}</p>
                </CardContent>
              </Card>
            </div>
          </section>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-2xl font-semibold">Compare Metrics</h2>
              <Button variant={showCompare ? 'default' : 'outline'} size="sm" onClick={() => setShowCompare(!showCompare)}>
            ...
            {showCompare && (
              <UICard className="p-4">
                <div className="flex flex-wrap items-center gap-4 mb-2 text-sm">
                  <label className="flex items-center gap-2"><Checkbox checked={!!series.temp} onCheckedChange={() => setSeries(s=>({...s, temp: !s.temp}))} /> Temperature</label>
                  <label className="flex items-center gap-2"><Checkbox checked={!!series.hum} onCheckedChange={() => setSeries(s=>({...s, hum: !s.hum}))} /> Humidity</label>
                  <label className="flex items-center gap-2"><Checkbox checked={!!series.wind} onCheckedChange={() => setSeries(s=>({...s, wind: !s.wind}))} /> Wind</label>
                  <label className="flex items-center gap-2"><Checkbox checked={!!series.uv} onCheckedChange={() => setSeries(s=>({...s, uv: !s.uv}))} /> UV</label>
                </div>
                <div className="h-64">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
              {weatherData.forecast.map((forecast, index) => {
                ...
                return (
                  <Card key={index} className="text-center cursor-pointer hover:shadow-glow" onClick={() => navigate(`/weather/${forecast.day.toLowerCase()}`)}>
                    <CardContent className="p-4">
                      <div className="font-medium text-sm mb-2">{forecast.day}</div>
                      <Icon className="h-8 w-8 mx-auto mb-2 text-primary" />
                      <div className="text-xs text-muted-foreground mb-1">{forecast.desc}</div>
